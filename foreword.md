...
通过 render 方法将一个 VNode 渲染至 DOM 中，其中我们的目的需要执行组件的逻辑，而不需要将组件渲染进页面的 DOM 树中
...
const onSubmit = async () => {
  ...
  window.setTimeout(() => {
      // 3. 1s 过后，组件中的数据加载完成，跳转至 content 页面
      router.push('/content')
  }, 1000)
}
</script>
...
目前，仅仅是内存中加载了 Content 组件并不会有缓存的效果，因为在路由跳转后，VueRouter 又会重新渲染一个全新的 Content 组件，和我们在内存中预加载的 Content 没有任何联系
...
const myComponent: Component = {
  ...
  setup(props) {
    ...
    return () => {
      ...
      // 找到预先加载的组件了，返回这个内存中的组件，且这个组件的 shapeFlag 是带有 “512” 标识的
